#!/usr/bin/env bash
# ============================================================
# Pi-hole Full Deployment Script
# Deploys pi_hole_web_survillance + pihole-mcp-server to Pi
#
# Usage:
#   cp 3_deploy_example.sh 3_deploy.sh
#   # Edit PI_HOST and PI_USER below, then:
#   bash 3_deploy.sh          # full deploy
#   bash 3_deploy.sh --update # quick update only (no reinstall)
# ============================================================
set -euo pipefail

# ── Config — set these before running ─────────────────────
PI_HOST="YOUR_PI_IP"          # e.g. 192.168.1.100
...
[[ "$PI_HOST" == "YOUR_PI_IP" ]] && error "Set PI_HOST to your Pi's IP address first"
...
do
    [[ -f "$f" ]] || error "Missing config: $f — copy from .example.yaml and fill in values"
...
ssh -o ConnectTimeout=10 -o BatchMode=yes "$PI_SSH" "echo ok" > /dev/null 2>&1 \
    || error "Cannot SSH to ${PI_SSH}. Check the Pi is on and SSH keys are set up."
ok "SSH connection to ${PI_SSH} working"

# ─────────────────────────────────────────────────────────
section "Part 1 — pi_hole_web_survillance (Analytics Dashboard)"
# ─────────────────────────────────────────────────────────
...
if [[ "$UPDATE_ONLY" == true ]]; then
    info "Update mode — restarting dashboard only..."
    ssh "$PI_SSH" "sudo systemctl restart pihole-analytics-dashboard"
    ok "Dashboard restarted"
else
    info "Running installer on Pi (first run takes ~2 min)..."
...
# ─────────────────────────────────────────────────────────
section "Part 2 — pihole-mcp-server (Telegram Bot)"
# ─────────────────────────────────────────────────────────
...
if [[ "$UPDATE_ONLY" == false ]]; then
    ...
    " || {
        ...
        ssh "$PI_SSH" "pip3 install --break-system-packages --quiet -r ${MCP_DEST}/requirements.txt"
    }
...
if [[ "$DASHBOARD_STATUS" == "active" ]]; then
    ...
    warn "pihole-analytics-dashboard: ${DASHBOARD_STATUS} — check: journalctl -u pihole-analytics-dashboard -n 20"
fi

if [[ "$MCP_STATUS" == "active" ]]; then
    ok "pihole-mcp-watch (Telegram bot): active"
else
    warn "pihole-mcp-watch: ${MCP_STATUS} — check: journalctl -u pihole-mcp-watch -n 20"
fi
...
if [[ "$UPDATE_ONLY" == false ]]; then
    ...
    if [[ "${QUERY_COUNT:-0}" -gt 0 ]]; then
        ...
        warn "Database has 0 queries — fetcher may still be running (check in 1 min)"
    fi
fi
